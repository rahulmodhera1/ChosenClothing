    ...
    <section ref={ref} className="py-20 bg-[#0a0906]">
      <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="flex items-end justify-between mb-8">
          ...
          <Link href="/lookbook" className="hidden sm:inline-block text-[#c4a882] text-xs tracking-widest uppercase border-b border-[#c4a882] pb-0.5 hover:text-[#f0ebe3] hover:border-[#f0ebe3] transition-colors">View All</Link>
        </motion.div>
      </div>
      <div className="flex gap-2 overflow-hidden">
        {teaserImages.map((img, i) => (
          <motion.div key={i} initial={{ opacity: 0, scale: 1.05 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.7, delay: i * 0.1 }} className="relative flex-1 min-w-0 aspect-[2/3]">
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto mt-6 sm:hidden">
        <Link href="/lookbook" className="text-[#c4a882] text-xs tracking-widest uppercase border-b border-[#c4a882] pb-0.5">View All</Link>
      </div>
    </section>
  );
}
